...
export default function StudentPortal() {
  const { user } = useAuth()
  const { pageConfig, loading, error } = usePageConfig('student-portal')
  ...
  return (
    <StudentGuard>
        ...
        <ResponsiveDrawer drawerContent={drawerContent}>
          <div className="min-h-screen flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-600">The student portal configuration could not be loaded.</p>
            </div>
          </div>
        </ResponsiveDrawer>
      )}
      ...
      {!loading && !error && pageConfig && (
        <ResponsiveDrawer drawerContent={drawerContent}>
          <DynamicRenderer pageConfig={pageConfig} userData={studentData} />
        </ResponsiveDrawer>
      )}
    </StudentGuard>
  )
